...
Related Human SOP: FOUND-001\_SimpliCreative\_SimpliFoundation\_Human\_SOP\_v1.2.docx
...
APPROVED STRATEGY  
 ...
 → DEVELOPMENT HANDOFF  
 → BUILD  
 → STRATEGIC FIDELITY QA & LAUNCH READINESS  
 → LAUNCH  
 → LAUNCH RECORD & POST-LAUNCH VALIDATION  
 → IMPLEMENTATION & ONGOING OWNERSHIP HANDOFF  
 → FOUNDATION COMPLETE
...
# **5A. Foundation Output Generation Checklist**

Use this checklist as the engineering production contract for standard Foundation outputs. Generate an output only when its trigger is satisfied, the engagement scope includes the deliverable, and the required upstream context is available. Missing upstream context is a blocking condition or explicit open item, not permission to infer.

Production labels: AI-PRODUCED means AI may assemble a substantial first draft from governed sources. AI-ASSISTED / HUMAN-LED means AI structures and proposes, but human judgment determines the substantive result. AI-ASSISTED / HUMAN-EXECUTED means AI prepares checks or records, while humans perform the real-world validation.

| ID / Output | Generation Trigger | Minimum Required Inputs | AI Role | Approval Gate | Output Contract |
| :---- | :---- | :---- | :---- | :---- | :---- |
| GS-FOUND-001 Strategy-to-Execution Handoff | Foundation activated with approved upstream strategy. | Approved Blueprint or validated strategy; objectives; buyer; recommendations; dependencies; constraints; open questions; scope. | AI-PRODUCED | Strategist \+ Project Lead | DOCX | INTERNAL | Draft until approved. Must preserve approved strategy, implementation priorities, dependencies, and unresolved items. |
| GS-FOUND-002 Website Messaging & Content Blueprint | Website messaging/content strategy is in scope and governing strategy exists. | Approved strategy; buyer architecture; buyer intent; messaging direction; proof/claims; site/content context; discoverability; measurement; client decisions. | AI-PRODUCED | Strategist \+ Client where material direction is client-facing | DOCX | CLIENT \+ INTERNAL | Must distinguish approved strategy, client-provided information, implementation requirements, illustrative copy, and open/verify items. |
| GS-FOUND-003 Brand Voice & Editorial Guide | Durable editorial guidance is in scope. | Approved messaging; brand standards; approved client content; client feedback/preferences; vocabulary; claim rules; buyer/channel context; editorial standards. | AI-PRODUCED | Strategist; Client where required | DOCX | CLIENT \+ INTERNAL | Governs expression, not strategy. Must include AI-writing controls when AI-assisted content is in scope. |
| GS-FOUND-004 Page Content & Messaging Brief | Material page is approved in architecture and needs copy/design planning. | Approved strategy; Integrated Site Architecture page record; buyer intent; page objective; Website Messaging Blueprint; Brand Voice; proof; measurement; current decisions. | AI-PRODUCED | Strategist | DOCX | INTERNAL | Must separate requirements from illustrative copy and preserve page objective, proof, CTA, intent, dependencies, and open decisions. |
| GS-FOUND-005 Website Copy Draft | Page brief is approved and copy is in scope. | Approved page brief; Brand Voice; Website Messaging Blueprint; approved proof/claims; buyer intent; CTA; current client decisions. | AI-PRODUCED FIRST DRAFT | Human editorial review; Client approval before final where required | DOCX | WORKING → CLIENT REVIEW | May improve expression; may not invent strategy, proof, offer terms, buyer priority, or performance claims. |
| GS-FOUND-006 Creative Brief | Design is in scope and page/content direction is sufficiently approved. | Approved strategy; Integrated Site Architecture; page requirements; buyer intent; messaging; approved/working copy; proof; conversion; accessibility; functional requirements; brand standards. | AI-PRODUCED | Strategist \+ Creative Lead | DOCX | INTERNAL | Must give designer enough context to design without reconstructing strategy. Final visual solution remains human-led. |
| GS-FOUND-007 Development Handoff & Implementation Specification | Approved design exists or implementation is ready for technical specification. | Approved design; approved copy; site architecture; page requirements; CMS; components; forms; CRM; measurement; metadata/schema; accessibility; migration; redirects; dependencies. | AI-PRODUCED FROM APPROVED SOURCES | Strategist \+ Designer \+ Developer for relevant areas | DOCX | INTERNAL | Design-dependent fields must remain PENDING APPROVED DESIGN when design evidence is absent. No invented pixel-level specs. |
| GS-FOUND-008 Integrated Site Architecture | Architecture/redesign/restructure/migration/page-strategy work is in scope. | Approved strategy; buyer \+ buyer intent; existing URL/nav inventory; page objectives; offers; proof; search evidence; content inventory; measurement; CRM/funnel context; constraints; scope. | AI-ASSISTED / HUMAN-LED | Strategist; Client for material architecture decisions | XLSX | CLIENT \+ INTERNAL | Sitemap tab is page-level source of truth. Includes URLs, intent, page job, CTAs, metadata, schema, dependencies, measurement, redirects/supporting tabs. No invented schema eligibility. |
| GS-FOUND-009 Strategic Fidelity QA & Launch Readiness | Implementation is sufficiently complete for formal pre-launch QA. | Approved strategy; architecture; page requirements; copy; design; development requirements; claims; forms/CRM; analytics; accessibility; current staging implementation. | AI-ASSISTED / HUMAN-EXECUTED | Strategist for fidelity; technical specialists for technical QA; Project Lead / authorized human for readiness | XLSX | INTERNAL | EXPECTED\_STATE → OBSERVED\_STATE → EVIDENCE → STATUS → SEVERITY → OWNER → RESOLUTION. PASS requires evidence. NOT TESTED \!= PASS. |
| GS-FOUND-010 Launch Record & Post-Launch Validation | Human-authorized production deployment has occurred. | Approved launch state; production URL/release; accepted limitations; issue register; live forms/CRM; analytics; redirects; indexing; integrations; responsive/accessibility evidence. | AI-ASSISTED / HUMAN-EXECUTED | Project / Technical Lead; authorized human owns launch state | XLSX | INTERNAL | Records what actually launched. Live observations require live evidence. NOT VALIDATED \!= PASS. |
| GS-FOUND-011 Implementation & Ongoing Ownership Handoff | Foundation is closing and ongoing ownership must transition. | Final approved deliverables; production state; systems/integrations; access/ownership; measurement; known limitations; open actions; ongoing responsibilities; selected ownership route. | AI-PRODUCED | Project Lead; Client confirms ongoing route | DOCX | CLIENT \+ INTERNAL TRANSITION | Must state delivered assets, owners, limitations, open items, operating guidance, and route to SimpliCARE or client/partner ownership. |

Engineering rule: outputs are stateful governed artifacts, not free-form generations. Store draft/approval status, source references, client boundary, output version, and human approver. Do not automatically promote one output's AI draft into an approved input for the next output.
...
Standard output automation must respect the checklist in Section 5A. A downstream output may be generated from an upstream draft for working purposes only when the workflow explicitly permits it and the downstream artifact preserves the upstream draft status. Approved-state dependencies must never be silently bypassed.
...
APPROVED STRATEGY  
 ...
 HUMAN EXECUTION  
 ↓  
 STRATEGIC FIDELITY QA \+ TECHNICAL QA  
 ↓  
 LAUNCH READINESS  
 ↓  
 HUMAN-AUTHORIZED LAUNCH  
 ↓  
 LAUNCH RECORD \+ POST-LAUNCH VALIDATION  
 ↓  
